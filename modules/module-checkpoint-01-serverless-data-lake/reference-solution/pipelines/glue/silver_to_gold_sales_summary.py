...
from pyspark.sql.functions import (
    ...
    max as spark_max, countDistinct, when, lag, rank, dense_rank, row_number,
    to_date, year, month, dayofmonth, date_sub, coalesce, round as spark_round
)
...
def read_silver_products(
    ...
    try:
        ...
        df = dynamic_frame.toDF()
        logger.info(f"Successfully read Silver products", row_count=df.count())
        return df
        
    except Exception as e:
        logger.error(f"Failed to read Silver products", exception=e)
        raise
...
def write_gold_sales_summary(
    ...
    try:
        ...
        logger.error(f"Failed to write Gold sales summary", exception=e)
        raise
