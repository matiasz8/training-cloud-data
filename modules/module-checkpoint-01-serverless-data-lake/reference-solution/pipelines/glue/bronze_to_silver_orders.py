...
from pyspark.sql.functions import (
    ...
    from_unixtime, unix_timestamp, datediff, year, month, dayofmonth
)
...
def read_bronze_orders(glue_context: GlueContext, database: str, table: str, logger: GlueLogger) -> DataFrame:
    ...
    try:
        ...
        logger.info(
            f"Successfully read Bronze orders data",
            row_count=row_count,
        ...
        logger.error(f"Failed to read Bronze orders data", exception=e)
        raise
...
def write_silver_orders(
    ...
    try:
        ...
        logger.error(f"Failed to write Silver orders data", exception=e)
        raise
