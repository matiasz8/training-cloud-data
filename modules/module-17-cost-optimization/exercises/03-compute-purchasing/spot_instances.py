...
class SpotInstanceManager:
    ...
    def implement_spot_best_practices(self):
        ...
        print(f"\n   1. Diversification:")
        print(f"      • Use multiple instance types (m5, m5a, m4)")
        print(f"      • Spread across availability zones")
        print(f"      • Use capacity-optimized allocation strategy")
        
        print(f"\n   2. Interruption Handling:")
        print(f"      • Enable Spark speculation (re-run slow tasks)")
        print(f"      • Checkpoint to S3 every 5-10 minutes")
        print(f"      • Set task retries (maxFailures=10)")
        print(f"      • Use EMR Managed Scaling")
        
        print(f"\n   3. Bidding Strategy:")
        print(f"      • Bid at On-Demand price for best availability")
        print(f"      • Don't bid low to save (causes frequent interruptions)")
        print(f"      • Use capacity-optimized vs lowest-price")
        
        print(f"\n   4. Architecture:")
        print(f"      • Master: Always On-Demand (cluster stability)")
        print(f"      • Core: On-Demand (data persistence in HDFS)")
        print(f"      • Task: Spot (processing only, no data storage)")
        
        print(f"\n   5. Monitoring:")
        print(f"      • Track interruption rate (target <5%)")
        print(f"      • Monitor spot price trends")
        print(f"      • Set CloudWatch alarms for capacity issues")
...
def main():
    ...
    print(f"\n📊 Key Takeaways:")
    print(f"   • Spot Instances: 70-90% cheaper than On-Demand")
    print(f"   • Best for: Batch processing, EMR, containerized workloads")
    print(f"   • Interruption rate: <5% with capacity-optimized strategy")
    print(f"   • Diversification: Use 3-5 instance types for resilience")
    
    print(f"\n💡 Implementation Steps:")
    print(f"   1. Start with non-critical batch workloads")
    print(f"   2. Implement checkpointing and retries")
    print(f"   3. Monitor interruptions and adjust")
    print(f"   4. Gradually increase Spot percentage (target 70-80%)")
    print(f"   5. Keep 20-30% On-Demand for stability")
    
    print(f"\n⚠️  When NOT to Use Spot:")
    print(f"   ❌ Production APIs (latency-sensitive)")
    print(f"   ❌ Stateful applications (databases with local storage)")
    print(f"   ❌ Jobs that can't tolerate interruptions")
    print(f"   ❌ Workloads requiring SLA guarantees")
