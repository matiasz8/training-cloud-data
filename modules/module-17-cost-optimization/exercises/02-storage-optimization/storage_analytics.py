...
class StorageAnalyticsManager:
    ...
    def identify_optimization_opportunities(self, df: pd.DataFrame) -> List[Dict]:
        ...
        if opportunities:
            ...
            print(f"✅ No major optimization opportunities found")
    ...
    def generate_storage_report(self, df: pd.DataFrame, opportunities: List[Dict]):
        ...
        print(f"\n📁 Reports saved:")
        print(f"   - storage-optimization-report.csv")
        if opportunities:
            print(f"   - storage-opportunities.csv")
...
def main():
    ...
    if df.empty:
        print(f"⚠️  No buckets found or unable to analyze")
        sys.exit(0)
    ...
    if opportunities:
        ...
        print(f"     1. Review opportunities in storage-opportunities.csv")
        print(f"     2. Start with High priority buckets (largest savings)")
        print(f"     3. Use lifecycle_policies.py to configure policies")
        print(f"     4. Use intelligent_tiering.py for unpredictable access")
        print(f"     5. Monitor with S3 Storage Lens after 24 hours")
    else:
