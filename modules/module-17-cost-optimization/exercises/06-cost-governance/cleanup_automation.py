...
def main():
    ...
    print(f"   1. Start with DRY RUN mode (test for 1-2 weeks)")
    print(f"   2. Target dev/test environments only initially")
    print(f"   3. Use tag-based scoping (Environment=dev)")
    print(f"   4. Schedule during low-activity hours (2-4 AM)")
    print(f"   5. Send notifications to team Slack channels")
    print(f"   6. Keep 'Keep' tag exception for critical resources")
    
    print(f"\n📅 Recommended Cleanup Schedule:")
    print(f"   • Daily: Stop idle dev/test EC2 instances")
    print(f"   • Weekly: Delete unattached volumes >30 days")
    print(f"   • Monthly: Delete old snapshots >90 days")
    print(f"   • Quarterly: Review and cleanup unused AMIs")
    
    print(f"\n⚡ Deploy as Lambda:")
    print(f"   Runtime: Python 3.11")
    print(f"   Memory: 256 MB")
    print(f"   Timeout: 300 seconds (5 minutes)")
    print(f"   Trigger: EventBridge cron(0 2 * * ? *) - Daily 2 AM UTC")
    print(f"   IAM: Limited to dev/test resources with tag conditions")
    
    print(f"\n📊 Expected Savings:")
    print(f"   • 10-20% reduction in total AWS spend")
    print(f"   • Typical org: $50K/month → $5-10K/month saved")
    print(f"   • ROI: Implementation 8 hours, continuous savings")
