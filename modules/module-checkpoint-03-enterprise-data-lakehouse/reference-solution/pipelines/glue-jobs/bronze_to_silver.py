...
from pyspark.sql.functions import (
    ...
    regexp_replace, when, coalesce, to_date, to_timestamp,
    concat, concat_ws, substring, length, initcap,
    year, month, dayofmonth, row_number, lead, lag,
    sum as spark_sum, count, max as spark_max, min as spark_min,
    md5, sha2, datediff, current_date
)
from pyspark.sql.types import StringType, IntegerType, DoubleType, DateType
from delta.tables import DeltaTable

from common.spark_utils import (
    create_spark_session, read_from_s3, write_to_delta,
    optimize_table, collect_stats, send_notification,
    ...
    def apply_scd_type2(
        ...
        try:
            ...
            changed_records = new_data.alias("new").join(
                ...
                f"old.is_current = false AND old.end_date = current_date()"
            ).select("new.*")
